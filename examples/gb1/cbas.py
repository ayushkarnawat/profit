...
iterations).
TODO: Should we save the samples (oracle and gt) for each iteration, so
that we can plot the 80th percentile for each iteration. The problem
with this is that it might lead to huge dumps since we have to store all
num_samples * num_iterations + original dataset for each of the
different runs, which might be unnecessary for most tasks. We could
subset each experiment in its own dump, but even then will result in a
huge file to process over. However, one benefit of this approach is that
this dump allows for more flexible stats since (a) we can compute stats
for each iteration, (b) compute the topk for all sequences by flattening
and sorting the array, and (c) use the files to see how each
"experiment" results in oracle and GT values change, with the oracle
values being sorted by the iteration number. The hope is then, to
observe that the top1 values are generally there in each iteration.
Additionally, it allows us to plot the sequences on a xy-grid, with the
z-value denoting the fitness score, and the size of the point denoting
the deviation from the GT? (if we even want that).
TODO: Is it better to have more iterations, but less samples per
iteration? I speculate that more iterations will leads the VAE to sample
more effectively, since it has to try less. But, on the other hand,
haivng more samples per iteration might help explore sequences which
might not have been generated by the latent representation????
...
def cbas(oracles: typing.List[SequenceOracle], gp: SequenceGPR, vae: SequenceVAE,
         vae_0: SequenceVAE, verbose: bool = False):
    """Full CbAS procedure.
    ...
    num_iters = 50      # num iterations, how long do we want to run this for?
    num_samples = 200   # num of new seqs to sample per iteration
    ...
    # Bookkeeping
    save_train_seqs = False # save seqs from initial dataset?
    tracker = {
    ...
    for t in range(num_iters):
        ...
        if verbose:
            ...
            print(t, tracker["seq"][t, top1_idx],
                  tracker["y_gt"][t, top1_idx],
...
track = cbas(all_oracles["g-mean"], all_gps["g-mean"], vae=vae, vae_0=vae_0,
             verbose=1)
print(track)
...
#         tracker["topk"]["seq"] = tracker["topk"]["seq"].tolist()
#         tracker["topk"]["y_gt"] = tracker["topk"]["y_gt"].numpy().tolist()
#         tracker["topk"]["y_oracle"] = tracker["topk"]["y_oracle"].numpy().tolist()
#         tracker["stats"] = tracker["stats"].numpy().tolist()
#         dump[f"oracle={oracle_metadata}"][f"gp={gp_metadata}"] = tracker
