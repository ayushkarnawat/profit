...
class CacheNamePolicy(object):
    ...
    def __init__(self,
        ...
        num_data_str = f"{num_data}" if num_data >= 0 else ""
        mutator_str = f"{mutator.lower()}" if mutator else "original"
        vocab_str = kwargs.get("vocab", None)
        self.filename = f"{mutator_str}{num_data_str}_encoding={vocab_str}.{filetype}"
        self.method = method
        self.labels = labels
