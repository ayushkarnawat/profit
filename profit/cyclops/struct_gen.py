...
def gen_all_pos_peptides(pepliblen):
    ...
    acids read in by the loadaminodata() function.          
    '''
...
def constrained_peptide_smiles(peptideseq, pattern):
     ...
     for resi, code in zip(peptideseq, pattern[2:]):
          ...
          if code in codes.keys():
               ...
               raise BondSpecError('%s in pattern %s not recognised' % (code, pattern))
...
def write_molecule(smiles, peptideseq, bond_def, outfldr, type='sdf',
     ...
     if write == 'draw':
          ...
          # In order to get good 3D conformations, it is a good idea to add H's to molecule first, 
          # embed (write) the coordinates, and then remove the unwanted H's. 
          mymol = AllChem.AddHs(mymol)
          AllChem.EmbedMolecule(mymol)
          AllChem.UFFOptimizeMolecule(mymol)
          mymol = AllChem.RemoveHs(mymol)
